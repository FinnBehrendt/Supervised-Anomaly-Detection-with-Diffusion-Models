...
from src.utils.SSIM_TM import StructuralSimilarityIndexMeasure as SSIM

def _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label_vol, cam=None) :
        ...
        ssim_met = SSIM(return_full_image=True,reduction='none',sigma=self.cfg.get('ssim_sigma',1)) # torchmetrics implementation  
        ssim_agg, ssim = ssim_met(final_volume.type_as(data_orig),data_orig)

        # calculate the residual image
        if self.cfg.get('residualmode','ssim')=='l1': # l1 or l2 residual
            diff_volume = torch.abs((data_orig-final_volume))
        elif self.cfg.get('residualmode','ssim')=='ssim':
            ssims_diff = (1 - ssim) * data_mask
            diff_volume = ssims_diff
        elif self.cfg.get('residualmode','ssim')=='l2':
            diff_volume = (data_orig-final_volume)**2
        else:
            raise NotImplementedError
        
        diff_volume = postprocess_vol(diff_volume.squeeze(),cfg=self.cfg) # postprocess the difference image
        if self.cfg.get('lambda_cam',None) is not None: # canoical combination of cam and residual image (experiments in appendix)
            diff_volume = self.cfg.get('lambda_cam',None) * diff_volume * data_mask + (1-self.cfg.get('lambda_cam',None)) * cam.unsqueeze(0) * data_mask if len(cam.shape) != len(diff_volume) else self.cfg.get('lambda_cam',None)* diff_volume * data_mask + (1-self.cfg.get('lambda_cam',None)) * cam * data_mask
        else: 
            diff_volume = diff_volume * data_mask + cam.unsqueeze(0) * data_mask if len(cam.shape) != len(diff_volume) else diff_volume * data_mask + self.cfg.get('lambda_cam',None) * cam * data_mask
...
def postprocess_vol(diff_volume, cfg):
    # median filter
    return torch.from_numpy(apply_3d_median_filter(diff_volume.numpy().squeeze(),kernelsize=cfg.get('kernelsize_median',5))).unsqueeze(0) # bring back to tensor